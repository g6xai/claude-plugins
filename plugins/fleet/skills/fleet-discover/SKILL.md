...
The manifest MUST conform to the JSON schema defined alongside this skill at `plugins/fleet/skills/fleet-discover/manifest.schema.json`.

Read that file before writing the manifest and follow its structure exactly. It pins:
- `fleet_version` must equal `"1.0.0"`
- Required top-level fields: `fleet_version`, `generated_at`, `scan_strategy`, `project`, `packages`, `size`
- Enums for `scan_strategy`, `project.type`, `packages[].type`, `infrastructure.database.type`, `size.confidence`, etc.
